...
const Rproducts = () => {
  ...
  useEffect(() => {
    axios.get('https://dummyjson.com/products?limit=4&skip=0&select=title,price,thumbnail,rating,category,discountPercentage')
      .then((res) => {
        setScoproduct(res.data.products)   // ✅ FIXED
      })
      .catch((err) => console.log(err))
  }, [])
    ...
    <>
      <section className='reproduct'>
        <div className="container py-[88px]">
          <div>
            <h2 className='font-Poppin font-semibold text-[36px] text-[#4B5563]'>
              <span className='text-[#111827] font-Poppin font-semibold text-[36px]'>
                Recommendations
              </span>
        ...
        <div className="container py-[52px]">
          <div className='bg-[#F5F5F5] flex justify-between rounded-[24px]'>
            <div className='py-[93px] w-[552px] px-[100px]'>
              ...
              <h2 className='text-[36px] w-[362px] font-Poppin font-medium text-fontcol'>The All New Fashion Collection Items</h2>
              <p className='text-[20px] font-Poppin font-medium text-fontcol'>Starting from: $59.99</p>
              <button className='p-[25px] w-[144px] h-[52px] active:scale-[1.1] bg-fontcol text-[16px] font-medium flex justify-center items-center mt-6 text-white rounded-[100px]'>
                shop now
              </button>
            </div>
